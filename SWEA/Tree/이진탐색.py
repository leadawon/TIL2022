...
def inorder_traverse(index,num,size,arr):
    if index <= size:
        num = inorder_traverse((index)*2,num,size,arr)
        arr[index] = num
        num += 1
        num = inorder_traverse((index)*2+1,num,size,arr)
    return num
...
for i in range(t):
    inpu = (int(input().rstrip()))
    arr = [0]*(inpu+1)
    inorder_traverse(1,1,inpu,arr)
    print(f"#{i+1} {arr[1]} {arr[inpu//2]}")
